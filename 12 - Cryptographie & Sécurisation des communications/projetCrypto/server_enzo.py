...
def diffuser(message, client_envoyeur=None):
    for indice, client in enumerate(clients):  # Parcours de tous les client
        if client != client_envoyeur:# Verification, on envoie que si ce n'est pas le client envoyeur
            pseudo = pseudos[indice]
            chat = f"{pseudo} : {message}"
...
def gestion(client):
    while True:  # Boucle infinie pour ecouter en permanence
        try:
            ...
            pseudos.pop(index)  # retirer le pseudo du client de la liste pseudos
            break
...
def connexions():
    while True:  # Boucle infinie pour ecouter en permanence
        ...
        pseudo = client.recv(1024).decode(
            "utf-8"
        )  # La premiere chose qu'enverra le client est son pseudo, nous pouvons donc traite ce premier messsage
